...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2] ([B*T,dim,grid ** 2])
        ...
        if self.joint:
            ...
            x = x[:,1:]
            # dimension combination [B*T, num_grid, c]->[B*num_grid, T, c]
            x = rearrange(x, '(b t) n c -> (b n) t c',b=B,t=self.T) 
            x = x + self.time_embedding.to(x.dtype)   # temporal pos emb
            # dimension combination [B*num_grid, T, c]->[B, num_grid*T, c]
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND  x: [num_grids+1, B*T, c]
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD  x: [B*T, num_grids+1, c]

        x = self.ln_post(x[:, 0, :]) # only cls tokens
