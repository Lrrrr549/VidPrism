...
class TemporalExpert(nn.Module):
    ...
    @torch.no_grad()
    def get_attn_weights(self, x: torch.Tensor, average_heads: bool = False):
        """
        x: [L, B, D]
        return:
          average_heads=False -> [B, H, L, L]
          average_heads=True  -> [B, L, L]
        """
        _, w = self.attn(x, x, x, need_weights=True, average_attn_weights=average_heads)
        return w
...
class VidPrism(nn.Module):
    def __init__(self,
                 num_experts: int = 4,
                 sampling_rates: List[int] = [2, 4, 8, 16],  # Sampling rate for each expert
                 num_classes: int = 101,                     # Number of classes, kept for interface compatibility
                 d_model: int = None,
                 dtem_tau: float = 0.1,
        ...
        # Identify which experts are "slow"
        is_slow_flags = [rate >= 8 for rate in sampling_rates]

        # Dynamic slow-fast interaction
        ...
        self.mote = MixtureOfTemporalExperts(
            ...
            num_experts=num_experts,
            expert_n_head=4,
            expert_d_ffn=d_model * 2
        )

        # Optional linear classifier head kept for interface compatibility
        self.head = nn.Linear(d_model, 400)

        # Projection head that maps video features into the shared semantic space
    ...
    def feature_level_pooling(self, features: torch.Tensor, importance_prior: torch.Tensor = None):
        """
        Args:
            features (torch.Tensor): Raw feature sequence (T, B, D)
        Returns:
            expert_inputs: Input sequence for each expert [T_i, B, D]
            extra_loss: Auxiliary ranking loss
        """
        features_for_pooling = features.permute(1, 2, 0)  # -> (B, D, T)
        if importance_prior is not None:
            importance_prior = importance_prior.detach()

        expert_inputs, extra_losses = [], []
        for pooler in self.pooling_layers:
            pooled_features, aux_loss = pooler(features_for_pooling)
            pooled_features = pooled_features.permute(2, 0, 1)  # -> (T_new, B, D)
            expert_inputs.append(pooled_features)
            extra_losses.append(aux_loss)

        extra_loss = torch.stack(extra_losses).mean() if extra_losses else features.new_tensor(0.0)
    ...
    def forward(self,
                ...
                batch_size: int = None,
                num_frames: int = None,
                importance_prior: torch.Tensor = None,
                text_embeds: torch.Tensor = None):
        """
        Args:
            video_frames: (B, T, C, H, W)
            text_embeds: (C, D) text-encoder output when running in CLIP mode
        Returns:
            logits: [B, C] for CLIP similarity or [B, num_classes] for standard classification
            gating_weights: [B, N]
            projected_features: [B, D]
            div_loss, extra_loss
        """
        # 1. Flatten video frames into sequence features
        features = video_frames.view(batch_size, num_frames, -1).permute(1, 0, 2)  # (T, B, D)

        # 2. Run multi-expert pooling and dynamic interaction
        ...
        # 3. Fuse the mixture-of-experts outputs
        fused_output, gating_weights, E_bnd = self.mote(expert_inputs)
        div_loss = diversity_loss_experts(E_bnd)

        # 4. Project into the shared semantic space
        projected_features = self.projection_head(fused_output)  # [B, D]
        # 5. Compute logits
        if text_embeds is not None:
            # CLIP-style similarity computation
            vid_emb = F.normalize(projected_features, dim=-1)
            txt_emb = F.normalize(text_embeds, dim=-1)
            logits = vid_emb @ txt_emb.t()  # [B, C]
        else:
            print("No text embeddings")

        # Keep the output interface unchanged
        return logits, projected_features, gating_weights, div_loss, extra_loss, E_bnd
...
class VideoCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        return image_emb, self.logit_scale
