...
def init_distributed_mode(args):
    """
    初始化分布式模式。
    当只使用单GPU时，会自动关闭分布式，避免 NCCL 报错。
    """
    # 无 GPU 情况直接退出
    if not torch.cuda.is_available():
        print('[WARN] No CUDA device detected. Disable distributed mode.')
        args.distributed = False
        args.gpu = None
        return

    # 检测 pytorch launcher 的环境变量
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        args.rank = int(os.environ['RANK'])
        args.world_size = int(os.environ['WORLD_SIZE'])
        ...
        # 没有分布式环境 => 单卡模式
        print('[INFO] No distributed environment found. Using single GPU.')
        args.distributed = False
        args.rank = 0
        args.gpu = 0
        torch.cuda.set_device(args.gpu)
    ...
    # world_size=1 时，等价于单机单卡
    if args.world_size == 1:
        print('[INFO] Detected single GPU. Running in non-distributed mode.')
        args.distributed = False
        args.rank = 0
        args.gpu = 0
        torch.cuda.set_device(args.gpu)
    ...
    dist.barrier()
    print(f'[INFO] Distributed initialized: rank {args.rank}/{args.world_size}, GPU {args.gpu}', flush=True)
...
def reduce_tensor(tensor, n=None):
    ...
    if not (dist.is_available() and dist.is_initialized()):
        return tensor  # 单卡模式下，不需要 reduce，直接返回原值
...
def accuracy(output, target, topk=(1, )):
    ...
    # pred: [BS, maxk] (indeices of largest value)
    _, pred = output.topk(maxk, 1, True, True)
    pred = pred.t() # [maxk, BS]
    # correct: [maxk, BS] element = 1 if pred is correct
    correct = pred.eq(target.view(1, -1).expand_as(pred))
    # # ============== calculate the counts of wrong predictions ==============
    # false_pred = torch.masked_select(target,~(correct[:1].reshape(-1)))
    # false_cls, counts = torch.unique(false_pred,sorted=True,return_counts=True)
    # print('false classes:',false_cls)
    # print('false countes:',counts)
    # # =======================================================================
    res = []
    for k in topk:
