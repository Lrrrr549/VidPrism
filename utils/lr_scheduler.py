...
class WarmupLR(_LRScheduler):
    ...
    # built in function in torch.optim.lr_scheduler
    def get_lr(self):
        curr_lrs = []
...
class WarmupStepLR(_LRScheduler): 
    def __init__(self,
                 optimizer,
                 warmup_steps=0, # <--- 核心修改: warmup_epochs -> warmup_steps
                 warmup_powers=1,
                 warmup_lrs=0,
                 last_epoch=-1): # last_epoch 变量名保留，但现在它代表 "last_step"
        
        self.num_groups = len(optimizer.param_groups)
        
        # 使用 to_tuple 辅助函数，确保参数是元组
        # to_tuple 函数的实现需要确保它存在
        self.warmup_steps = to_tuple(warmup_steps, self.num_groups)
        self.warmup_powers = to_tuple(warmup_powers, self.num_groups)
        self.warmup_lrs = to_tuple(warmup_lrs, self.num_groups)

        super(WarmupStepLR, self).__init__(optimizer, last_epoch)
        # 基类的 self.base_lrs 会被自动设置
        assert self.num_groups == len(self.base_lrs)

    def get_lr(self):
        """
        这个函数在 scheduler.step() 时被调用，计算并返回新的学习率。
        self.last_epoch 在这里代表当前的 step 数。
        """
        curr_lrs = []
        for group_index in range(self.num_groups):
            # 检查当前 step 是否仍在 warmup 阶段
            if self.last_epoch < self.warmup_steps[group_index]:
                
                # --- Warmup 阶段的计算逻辑 ---
                # 避免除以0
                if self.warmup_steps[group_index] == 0:
                    progress = 1.0
                else:
                    progress = self.last_epoch / self.warmup_steps[group_index]
                
                factor = progress ** self.warmup_powers[group_index]
                lr_gap = self.base_lrs[group_index] - self.warmup_lrs[group_index]
                curr_lrs.append(factor * lr_gap + self.warmup_lrs[group_index])
            else:
                # --- Warmup 结束后，调用子类的计算逻辑 ---
                curr_lrs.append(self.get_single_lr_after_warmup(group_index))
        return curr_lrs
...
class WarmupCosineAnnealingStepLR(WarmupStepLR): # <--- 继承新基类并重命名

    def __init__(self,
                 optimizer,
                 total_steps, # <--- 从 total_epoch 改为 total_steps
                 final_factor=0,
                 warmup_steps=0, # <--- 从 warmup_epochs 改为 warmup_steps
                 warmup_powers=1,
                 warmup_lrs=0,
                 last_epoch=-1): # last_epoch 代表 last_step
        
        self.total_steps = total_steps
        self.final_factor = final_factor
        super(WarmupCosineAnnealingStepLR, self).__init__(optimizer,
                                                          warmup_steps,
                                                          warmup_powers,
                                                          warmup_lrs,
                                                          last_epoch)
    ...
    def get_single_lr_after_warmup(self, group_index):
        # self.warmup_steps 是一个元组，为每个参数组保存了warmup步数
        warmup_step_for_group = self.warmup_steps[group_index]
        
        # self.last_epoch 是当前步数
        # 计算余弦退火的进度
        # 避免除以0
        denominator = self.total_steps - warmup_step_for_group
        if denominator == 0:
            progress = 1.0
        else:
            progress = (self.last_epoch - warmup_step_for_group) / denominator
        
        progress = min(progress, 1.0) # 确保进度不超过1
...
class WarmupMultiStepStepLR(WarmupStepLR): # <--- 继承新基类并重命名

    def __init__(self,
                 optimizer,
                 milestones, # milestones 现在是以 step 为单位的
                 gamma=0.1,
                 warmup_steps=0, # <--- 改为 warmup_steps
                 warmup_powers=1,
                 warmup_lrs=0,
                 last_epoch=-1): # last_epoch 代表 last_step
        ...
        self.milestones = milestones
        self.gamma = gamma
        super(WarmupMultiStepStepLR, self).__init__(optimizer,
                                                    warmup_steps,
                                                    warmup_powers,
                                                    warmup_lrs,
                                                    last_epoch)
    ...
    def get_single_lr_after_warmup(self, group_index):
        # self.last_epoch 是当前步数
        # bisect_right 找到当前步数在 milestones 中的位置，以决定衰减多少次
        factor = self.gamma ** bisect_right(self.milestones, self.last_epoch)
        return self.base_lrs[group_index] * factor
